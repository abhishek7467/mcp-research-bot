...
<body>
    <div class="newspaper">
        ...
        <div class="footer">
            ...
            <p>© {year} - For research and educational purposes only.</p>
        </div>
    </div>
</body>
</html>
"""
    ...
    def _generate_article_html(self, item: Dict[str, Any]) -> str:
        ...
        if item.get('url'):
            html += f'<a href="{item["url"]}" target="_blank">📄 View Original</a>'
        if item.get('pdf_url'):
            html += f'<a href="{item["pdf_url"]}" target="_blank">📑 PDF</a>'
        if item.get('doi'):
            html += f'<a href="https://doi.org/{item["doi"]}" target="_blank">🔗 DOI</a>'
        html += '</div>'
